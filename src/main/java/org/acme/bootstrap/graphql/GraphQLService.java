  ...
  public Object testQl(@Context HttpHeaders httpHeaders) {
    ...
    ExecutionPlan plan = ExecutionPlan.builder().path("/api/market/areas").tree(tree)
        .selection(new AreaExecutionPlanner().withUid("codigo").withName("nombre")
            .withPlace(ofPlace -> ofPlace.withName("place.name")
                    .withMerchant("jefe", ofMerchant -> ofMerchant.withName("merchant.name")))
            .build())
        .build();

    List<Gon> list = runner.list(plan, Gon.class, Map.of(), httpHeaders.getRequestHeaders());
    list.forEach( gon -> {
