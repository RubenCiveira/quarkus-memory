...
public class CreateColorUsecase {
  ...
  private CompletionStage<Optional<Color>> createAndVerify(final ColorCreateCommand query,
    ...
    return gateway.create(colorEntity, created -> verifyCreation(query, created));
  }
   ...
   * @return
   */
  private CompletionStage<Boolean> verifyCreation(final ColorCreateCommand query,
      final Color created) {
    ColorFilter filter = ColorFilter.builder().uid(created.getUidValue()).build();
