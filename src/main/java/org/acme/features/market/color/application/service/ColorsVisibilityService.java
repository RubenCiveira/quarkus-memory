  ...
  public CompletionStage<Boolean> checkVisibility(Interaction prev, List<String> uids) {
    ...
    try (Scope scope = startSpan.makeCurrent()) {
      return listVisibles(prev, ColorFilter.builder().uids(uids).build(),
          ColorCursor.builder().build()).thenApply(list -> list.size() == uids.size())
              .whenComplete((val, ex) -> {
                if (null == ex) {
                  startSpan.setStatus(StatusCode.OK);
                } else {
  ...
  public CompletionStage<Set<String>> fieldsToFix(Interaction prev) {
    ...
    try (Scope scope = startSpan.makeCurrent()) {
      ...
      ColorFixedFieldsPipelineStageEvent value =
          ColorFixedFieldsPipelineStageEvent.builder().fields(fields).query(prev).build();
      fireFix.fire(value);
  ...
  public CompletionStage<Set<String>> fieldsToFix(Interaction prev, ColorRef color) {
    ...
    try (Scope scope = startSpan.makeCurrent()) {
      ...
      ColorFixedFieldsPipelineStageEvent value = ColorFixedFieldsPipelineStageEvent.builder()
          .fields(fields).color(color).query(prev).build();
      fireFix.fire(value);
      return value.getFields().whenComplete((val, ex) -> {
        if (null == ex) {
          startSpan.setAttribute("fieds", String.join(",", val));
  ...
  public CompletionStage<Set<String>> fieldsToHide(Interaction prev) {
    ...
    try (Scope scope = startSpan.makeCurrent()) {
      ...
      ColorHiddenFieldsPipelineStageEvent value =
          ColorHiddenFieldsPipelineStageEvent.builder().fields(fields).query(prev).build();
      fireHide.fire(value);
      return value.getFields().whenComplete((val, ex) -> {
        if (null == ex) {
          startSpan.setAttribute("fieds", String.join(",", val));
  ...
  private CompletionStage<ColorDto> visiblesReferences(Interaction prev, ColorDto source) {
    ...
    try (Scope scope = startSpan.makeCurrent()) {
      ...
      if (null != source.getMerchant()) {
        source.getMerchant().getReferenceValue().ifPresent(ref -> related.add(
            merchantsVisibilityService.checkVisibility(prev, ref).whenComplete((visible, fail) -> {
              if (!visible) {
