...
public class CreateColorUsecase {
  ...
  public CompletionStage<Allow> allow(final Interaction query) {
    Span startSpan = tracer.spanBuilder("color-create-allow").startSpan();
  ...
  public CompletionStage<ColorCreateResult> create(final ColorCreateCommand query) {
    Span startSpan = tracer.spanBuilder("color-create").startSpan();
    try (Scope scope = startSpan.makeCurrent()) {
      ...
      return create.thenCompose(color -> mapEntity(query, color)).whenComplete((val, ex) -> {
        if (null == ex) {
          startSpan.setStatus(StatusCode.OK);
        } else {
          startSpan.recordException(ex).setStatus(StatusCode.ERROR);
  ...
  private CompletionStage<Optional<Color>> createAndVerify(final ColorCreateCommand query,
    ...
    Span startSpan = tracer.spanBuilder("color-run-create-and-verify").startSpan();
    try (Scope scope = startSpan.makeCurrent()) {
      return gateway
  ...
  private CompletionStage<ColorCreateResult> mapEntity(final ColorCreateCommand command,
    ...
    Span startSpan = tracer.spanBuilder("color-map-created-entity").startSpan();
    try (Scope scope = startSpan.makeCurrent()) {
      return opcolor
