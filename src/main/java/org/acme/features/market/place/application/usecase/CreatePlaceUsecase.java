...
public class CreatePlaceUsecase {
  ...
  private CompletionStage<Optional<Place>> createAndVerify(final PlaceCreateCommand query,
    ...
    return gateway.create(placeEntity, created -> verifyCreation(query, created));
  }
   ...
   * @return
   */
  private CompletionStage<Boolean> verifyCreation(final PlaceCreateCommand query,
      final Place created) {
    PlaceFilter filter = PlaceFilter.builder().uid(created.getUidValue()).build();
