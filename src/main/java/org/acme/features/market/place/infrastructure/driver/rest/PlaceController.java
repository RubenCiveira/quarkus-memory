...
public class PlaceController implements PlaceApi {
  ...
  public Response placeApiList(final String uid, final List<String> uids, final String search,
    ...
    List<PlaceDto> listed = list.list(PlaceListQuery.builder().filter(filter.build())
        .cursor(cursor.build()).build(currentRequest.interaction()));
    /* .header("Last-Modified", value.getSince().format(DateTimeFormatter.RFC_1123_DATE_TIME)) */
    return Response.ok(new PlaceList().content(toApiModel(listed)).next(next(orderSteps, listed)))
        .build();
  }
