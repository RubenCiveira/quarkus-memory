...
import org.acme.features.market.place.application.usecase.update.event.PlaceUpdateAllowPipelineStageEvent;
...
public class UpdatePlaceUsecase {
   ...
   * @autogenerated UpdateUsecaseGenerator
   */
  private final Event<PlaceUpdateAllowPipelineStageEvent> updateAllow;
  ...
  public CompletionStage<Allow> allow(final Interaction query, final PlaceRef reference) {
    Span startSpan = tracer.spanBuilder("place-update-allow-specific").startSpan();
    try (Scope scope = startSpan.makeCurrent()) {
      PlaceUpdateAllowPipelineStageEvent base = PlaceUpdateAllowPipelineStageEvent.build(query,
  ...
  public CompletionStage<Allow> allow(final Interaction query) {
    Span startSpan = tracer.spanBuilder("place-update-allow-generic").startSpan();
    try (Scope scope = startSpan.makeCurrent()) {
      PlaceUpdateAllowPipelineStageEvent base = PlaceUpdateAllowPipelineStageEvent.build(query,
  ...
  public CompletionStage<PlaceUpdateResult> update(final PlaceUpdateCommand command) {
    Span startSpan = tracer.spanBuilder("place-update").startSpan();
    try (Scope scope = startSpan.makeCurrent()) {
  ...
  private CompletionStage<PlaceUpdateResult> mapEntity(final PlaceUpdateCommand command,
    ...
    Span startSpan = tracer.spanBuilder("place-map-updated-entity").startSpan();
    try (Scope scope = startSpan.makeCurrent()) {
      return opplace
  ...
  private CompletionStage<Optional<Place>> saveEntity(final Place original, final PlaceDto dto) {
    Span startSpan = tracer.spanBuilder("place-run-updated-entity").startSpan();
    try (Scope scope = startSpan.makeCurrent()) {
  ...
  private CompletionStage<Optional<Place>> saveIfIsPresent(final Optional<Place> result,
      final PlaceDto dto) {
    Span startSpan = tracer.spanBuilder("place-load-to-update-if-present").startSpan();
    try (Scope scope = startSpan.makeCurrent()) {
