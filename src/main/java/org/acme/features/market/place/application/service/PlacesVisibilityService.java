  ...
  public CompletionStage<Boolean> checkVisibility(Interaction prev, List<String> uids) {
    ...
    try (Scope scope = startSpan.makeCurrent()) {
      return listVisibles(prev, PlaceFilter.builder().uids(uids).build(),
          PlaceCursor.builder().build()).thenApply(list -> list.size() == uids.size())
              .whenComplete((val, ex) -> {
                if (null == ex) {
                  startSpan.setStatus(StatusCode.OK);
                } else {
  ...
  public CompletionStage<Set<String>> fieldsToFix(Interaction prev) {
    ...
    try (Scope scope = startSpan.makeCurrent()) {
      ...
      PlaceFixedFieldsPipelineStageEvent value =
          PlaceFixedFieldsPipelineStageEvent.builder().fields(fields).query(prev).build();
      fireFix.fire(value);
  ...
  public CompletionStage<Set<String>> fieldsToFix(Interaction prev, PlaceRef place) {
    ...
    try (Scope scope = startSpan.makeCurrent()) {
      ...
      PlaceFixedFieldsPipelineStageEvent value = PlaceFixedFieldsPipelineStageEvent.builder()
          .fields(fields).place(place).query(prev).build();
      fireFix.fire(value);
      return value.getFields().whenComplete((val, ex) -> {
        if (null == ex) {
          startSpan.setAttribute("fieds", String.join(",", val));
  ...
  public CompletionStage<Set<String>> fieldsToHide(Interaction prev) {
    ...
    try (Scope scope = startSpan.makeCurrent()) {
      ...
      PlaceHiddenFieldsPipelineStageEvent value =
          PlaceHiddenFieldsPipelineStageEvent.builder().fields(fields).query(prev).build();
      fireHide.fire(value);
      return value.getFields().whenComplete((val, ex) -> {
        if (null == ex) {
          startSpan.setAttribute("fieds", String.join(",", val));
  ...
  private CompletionStage<PlaceDto> visiblesReferences(Interaction prev, PlaceDto source) {
    ...
    try (Scope scope = startSpan.makeCurrent()) {
      ...
      if (null != source.getMerchant()) {
        related.add(merchantsVisibilityService
            .checkVisibility(prev, source.getMerchant().getReferenceValue())
            .whenComplete((visible, fail) -> {
              if (!visible) {
                throw new NotFoundException(
                    "No merchant - " + source.getMerchant().getReferenceValue());
              }
            }));
      }
