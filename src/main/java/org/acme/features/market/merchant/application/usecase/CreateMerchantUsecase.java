...
public class CreateMerchantUsecase {
  ...
  private CompletionStage<Optional<Merchant>> createAndVerify(final MerchantCreateCommand query,
    ...
    return gateway.create(merchantEntity, created -> verifyCreation(query, created));
  }
   ...
   * @return
   */
  private CompletionStage<Boolean> verifyCreation(final MerchantCreateCommand query,
      final Merchant created) {
    MerchantFilter filter = MerchantFilter.builder().uid(created.getUidValue()).build();
