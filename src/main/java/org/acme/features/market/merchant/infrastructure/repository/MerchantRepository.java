...
import org.acme.common.cypher.AesCipherService;
    ...
    return (row) -> {
      try {
        return Optional.of(Merchant.builder().uidValue(row.getString(1)).nameValue(row.getString(2))
            .enabledValue(row.getBoolean(3)).keyCypheredValue(row.getString(4))
            .versionValue(row.getInt(5)).build());
