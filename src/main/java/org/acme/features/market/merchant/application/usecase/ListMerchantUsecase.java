...
public class ListMerchantUsecase {
  ...
  public MerchantListResult list(final MerchantListQuery query) {
    CompletionStage<List<Merchant>> future = allow(query).getDetail().thenCompose(detail -> {
      ...
      return visibility.visibleFilter(query, query.getFilter()).thenCompose(visibleFilter -> {
        Slide<Merchant> slide = gateway.list(visibleFilter, query.getCursor());
   ...
   * @return The slide with some values
   */
  private CompletionStage<List<Merchant>> filterUnitLimit(final MerchantListQuery query,
      final List<Merchant> merchants) {
    return visibility.listableFilter(query, merchants);
  }
  ...
  private CompletionStage<List<MerchantDto>> mapList(final MerchantListQuery query,
    ...
    List<CompletableFuture<MerchantDto>> futures =
        merchants.stream().map(merchant -> visibility.hide(query, merchant))
