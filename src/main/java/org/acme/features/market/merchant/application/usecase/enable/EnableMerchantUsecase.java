...
public class EnableMerchantUsecase {
   ...
   */
  public BatchProgress checkProgress(final MerchantCheckBatchEnableStatusQuery query) {
   ...
   * @return The slide with some values
   */
  public BatchIdentificator enable(final MerchantEnableAllInBatchCommand command) {
    Allow detail = allow(command);
    if (!detail.isAllowed()) {
      throw new NotAllowedException(detail.getDescription());
    }
    return batch.start(command.getActor().getName().orElse("-"), Duration.ofHours(6), ExecutorPlan
        .<MerchantEnableAllInBatchCommand>builder().params(command).name("enable-color")
        .executor(
            ExecutorByDeferSteps.<Merchant, Merchant, MerchantEnableAllInBatchCommand, EnableMerchantsInBatchExecutor.MerchantPaginableBatch>builder()
                .initializer(EnableMerchantsInBatchExecutor.class)
                .counter(EnableMerchantsInBatchExecutor.class)
                .descriptor(EnableMerchantsInBatchExecutor.class)
                .reader(EnableMerchantsInBatchExecutor.class)
                .processor(EnableMerchantsInBatchExecutor.class)
                .writer(EnableMerchantsInBatchExecutor.class)
                .finalizer(EnableMerchantsInBatchExecutor.class).build())
        .build());
  }
   ...
   * @return
   */
  Merchant enable(final Interaction interaction, final Merchant original) {
    MerchantDto filled = MerchantDto.from(aggregate.enable(original));
    MerchantDto modified =
        MerchantEnableProposal.resolveWith(proposalEmitter, interaction, filled, original);
    Merchant saved =
        gateway.update(original, modified.toEntityBuilder(Optional.of(original)).buildValid());
    MerchantEnableEvent.notifyWith(eventEmitter, interaction, saved);
    return saved;
  }
