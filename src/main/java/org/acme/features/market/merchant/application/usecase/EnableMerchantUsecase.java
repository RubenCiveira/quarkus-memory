...
public class EnableMerchantUsecase {
  ...
  public MerchantEnableResult enable(final MerchantEnableCommand command) {
    CompletionStage<Optional<Merchant>> updated = allow(command).getDetail().thenCompose(detail -> {
      ...
      return retrieve(command, filter).thenCompose(op -> enableIfIsPresent(op));
    });
   ...
   * @return The slide with some values
   */
  private CompletionStage<Optional<Merchant>> retrieve(final MerchantEnableCommand command,
