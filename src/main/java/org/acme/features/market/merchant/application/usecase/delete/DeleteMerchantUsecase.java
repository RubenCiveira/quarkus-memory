...
public class DeleteMerchantUsecase {
  ...
  public CompletionStage<Allow> allow(final Interaction query, final MerchantRef reference) {
    Span startSpan = tracer.spanBuilder("merchant-specific-delete-allow").startSpan();
    try (Scope scope = startSpan.makeCurrent()) {
      MerchantDeleteAllowPipelineStageEvent base = MerchantDeleteAllowPipelineStageEvent
  ...
  public CompletionStage<Allow> allow(final Interaction query) {
    Span startSpan = tracer.spanBuilder("merchant-generic-delete-allow").startSpan();
    try (Scope scope = startSpan.makeCurrent()) {
      MerchantDeleteAllowPipelineStageEvent base = MerchantDeleteAllowPipelineStageEvent
  ...
  public CompletionStage<MerchantDeleteResult> delete(final MerchantDeleteCommand command) {
    Span startSpan = tracer.spanBuilder("merchant-delete").startSpan();
    try (Scope scope = startSpan.makeCurrent()) {
  ...
  private CompletionStage<Optional<Merchant>> deleteEntity(final Merchant original) {
    Span startSpan = tracer.spanBuilder("merchant-call-to-delete").startSpan();
    try (Scope scope = startSpan.makeCurrent()) {
  ...
  private CompletionStage<Optional<Merchant>> deleteIfIsPresent(final Optional<Merchant> result) {
    Span startSpan = tracer.spanBuilder("merchant-delete-if-present").startSpan();
    try (Scope scope = startSpan.makeCurrent()) {
  ...
  private CompletionStage<MerchantDeleteResult> mapEntity(final MerchantDeleteCommand command,
    ...
    Span startSpan = tracer.spanBuilder("merchant-map-response").startSpan();
    try (Scope scope = startSpan.makeCurrent()) {
      return opmerchant
