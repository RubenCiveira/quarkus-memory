...
public class MerchantController implements MerchantApi {
  ...
  public Response merchantApiList(final String uid, final List<String> uids, final String search,
    ...
    List<MerchantDto> listed = list.list(MerchantListQuery.builder().filter(filter.build())
        .cursor(cursor.build()).build(currentRequest.interaction()));
    /* .header("Last-Modified", value.getSince().format(DateTimeFormatter.RFC_1123_DATE_TIME)) */
    return Response
        .ok(new MerchantList().content(toApiModel(listed)).next(next(orderSteps, listed))).build();
  }
   ...
   * @return
   */
  private MerchantListNextOffset next(List<MerchantOrder> order, List<MerchantDto> list) {
    if (list.isEmpty()) {
      return null;
    } else {
      ...
      if (order.contains(MerchantOrder.NAME_ASC)) {
        next.setSinceName(last.getName().getValue());
      }
      if (order.contains(MerchantOrder.NAME_DESC)) {
        next.setSinceName(last.getName().getValue());
      }
      return next;
    }
