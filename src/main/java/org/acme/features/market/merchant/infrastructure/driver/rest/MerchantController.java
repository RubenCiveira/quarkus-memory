...
import org.acme.features.market.merchant.application.usecase.disable.MerchantCheckBatchDisableStatusQuery;
import org.acme.features.market.merchant.application.usecase.disable.MerchantDisableAllInBatchCommand;
...
import org.acme.features.market.merchant.application.usecase.enable.MerchantCheckBatchEnableStatusQuery;
import org.acme.features.market.merchant.application.usecase.enable.MerchantEnableAllInBatchCommand;
...
public class MerchantController implements MerchantApi {
   ...
   */
  @Override
  public Response merchantApiBatchDisable(final List<String> uids, final String search,
      final Boolean enabled, final String name) {
    MerchantFilter.MerchantFilterBuilder filterBuilder = MerchantFilter.builder();
    filterBuilder =
        filterBuilder.uids(uids.stream().flatMap(part -> Stream.of(part.split(","))).toList());
    ...
    filterBuilder = filterBuilder.name(name);
    MerchantFilter filter = filterBuilder.build();
    BatchIdentificator task = disable.disable(MerchantDisableAllInBatchCommand.builder()
        .filter(filter).build(currentRequest.interaction()));
    /* .header("Last-Modified", value.getSince().format(DateTimeFormatter.RFC_1123_DATE_TIME)) */
    return Response.ok(task).build();
  }
   ...
   */
  @Override
  public Response merchantApiBatchDisableQuery(final String batchId) {
    BatchProgress task = disable.checkProgress(MerchantCheckBatchDisableStatusQuery.builder()
        .taskId(batchId).build(currentRequest.interaction()));
    return Response.ok(task).build();
  }
   ...
   */
  @Override
  public Response merchantApiBatchEnable(final List<String> uids, final String search,
      final Boolean enabled, final String name) {
    MerchantFilter.MerchantFilterBuilder filterBuilder = MerchantFilter.builder();
    filterBuilder =
        filterBuilder.uids(uids.stream().flatMap(part -> Stream.of(part.split(","))).toList());
    ...
    filterBuilder = filterBuilder.name(name);
    MerchantFilter filter = filterBuilder.build();
    BatchIdentificator task = enable.enable(MerchantEnableAllInBatchCommand.builder().filter(filter)
        .build(currentRequest.interaction()));
    /* .header("Last-Modified", value.getSince().format(DateTimeFormatter.RFC_1123_DATE_TIME)) */
    return Response.ok(task).build();
  }
   ...
   */
  @Override
  public Response merchantApiBatchEnableQuery(final String batchId) {
    BatchProgress task = enable.checkProgress(MerchantCheckBatchEnableStatusQuery.builder()
        .taskId(batchId).build(currentRequest.interaction()));
    return Response.ok(task).build();
  }
  ...
  private MerchantDto toDomainModel(Merchant merchant) {
    ...
    if (null != merchant.getKey() && !"*****".equals(merchant.getKey())) {
