...
public class DeleteFruitUsecase {
  ...
  public FruitDeleteAllow allow(final FruitEntityAllowQuery query) {
    FruitDeleteAllow base = FruitDeleteAllow.build(true, "Allowed by default");
  ...
  public FruitDeleteResult delete(final FruitDeleteCommand command) {
    CompletableFuture<Optional<Fruit>> updated = allow(command).getDetail().thenCompose(detail -> {
      ...
      return gateway
          .retrieve(command.getReference().getUidValue(),
              Optional.of(visibility.visibleFilter(command, filter)))
