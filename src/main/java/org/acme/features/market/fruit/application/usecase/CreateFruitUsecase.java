...
public class CreateFruitUsecase {
  ...
  public FruitCreateResult create(final FruitCreateCommand query) {
    CompletableFuture<Optional<Fruit>> create = allow(query).getDetail().thenCompose(detail -> {
      ...
      return aggregate.initialize(visibility.copyWithFixed(query, query.getDto()).toEntityBuilder())
  ...
  private CompletableFuture<Boolean> verifyCreation(final FruitCreateCommand query,
      final Fruit created) {
    return gateway.exists(created.getUidValue(), Optional.of(
        visibility.visibleFilter(query, FruitFilter.builder().uid(created.getUidValue()).build())));
  }
}
