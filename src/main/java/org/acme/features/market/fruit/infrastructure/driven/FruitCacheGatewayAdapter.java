...
public class FruitCacheGatewayAdapter implements FruitCacheGateway {
   ...
   * @return
   */
  public FruitCached store(final FruitFilter filter, final FruitCursor cursor, final List<Fruit> fruits) {
    String key = key(filter, cursor);
    cache.invalidate(key).await().indefinitely();
    FruitCached value = cache
        .<String, FruitCached>get(key,
            k -> {
              System.out.println("ASKING FOR " + k);
              return FruitCached.builder().since(OffsetDateTime.now()).value(fruits).build();
            })
        .invoke(result -> System.out.println("CACHE RESULT: " + result))
        .await().indefinitely();
    return value;
  }
