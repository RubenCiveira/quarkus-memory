...
import org.acme.features.market.area.application.usecase.delete.AreaCheckBatchDeleteStatusQuery;
import org.acme.features.market.area.application.usecase.delete.AreaDeleteAllInBatchCommand;
...
public class AreaController implements AreaApi {
   ...
   */
  @Override
  public Response areaApiBatchDelete(final List<String> uids, final String search,
      final String name, final List<String> places) {
    ...
    filterBuilder =
        filterBuilder.places(places.stream().flatMap(part -> Stream.of(part.split(","))).toList());
    AreaFilter filter = filterBuilder.build();
    BatchIdentificator task = delete.delete(
        AreaDeleteAllInBatchCommand.builder().filter(filter).build(currentRequest.interaction()));
   ...
   */
  @Override
  public Response areaApiList(final List<String> uids, final String search, final String name,
      final List<String> places, final Integer limit, final String sinceUid, final String sinceName,
      final String order) {
    ...
    return currentRequest.cacheableResponse(listed, toListApiModel(listed, filter, cursor),
        "area-" + String.valueOf(("" + filter + cursor).hashCode()));
  }
  ...
  private AreaList toListApiModel(List<AreaDto> areas, AreaFilter filter, AreaCursor cursor) {
    ...
    if (!filter.getPlaces().isEmpty()) {
      String placesValue =
          URLEncoder.encode(String.join(",", filter.getPlaces()), StandardCharsets.UTF_8);
      self.append("&places=" + placesValue);
      next.append("&places=" + placesValue);
      first.append("&places=" + placesValue);
    }
